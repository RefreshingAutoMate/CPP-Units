...
namespace cppunits{
	...
	class AggregateUnit
	{
		...
		template<class G>
		AggregateUnit(AggregateUnit<G,N1,N2,N3,N4,N5,N6,N7> other_unit)
		{
			double temp = G::in_base_unit(other_unit.val);
			val = T::from_base(temp);
		}
		AggregateUnit(double _val): val(_val){}

	};
	
	template<typename T>
	class AggregateUnit<T, 0, 0, 0, 0, 0, 0, 0>
	{
		public:
			double val;
	...
	class SquaredUnit: public Unit
	{
		public:
			double val = std::pow(Unit::val, 2);

		// SquaredUnit(double _val): val(_val){}

		operator double()
		{
			return Unit::Prefix::in_base_unit(val) ;
		}
	};


	// template <typename CompilerPolicy, typename SystemPolicy>
	// class Compilation : private CompilerPolicy, private SystemPolicy {
	//     public:
	//         // Behavior method.
	//         void run() const 
	//         {
	...
	//     private:
	//         using SystemPolicy::Message;
	//         using CompilerPolicy::Print;
	// };
	...
	AggregateUnit<Base, N1, N2, N3, N4, N5, N6, N7> operator+(AggregateUnit<T1, N1, N2, N3, N4, N5, N6, N7> lhs, AggregateUnit<T2, N1, N2, N3, N4, N5, N6, N7> rhs){
		return AggregateUnit<Base, N1, N2, N3, N4, N5, N6, N7>(T1::in_base_unit(lhs.val) + T2::in_base_unit(rhs.val));
	}
	...
	AggregateUnit<Base, AN1+BN1, AN2+BN2, AN3+BN3, AN4+BN4, AN5+BN5, AN6+BN6, AN7+BN7> operator*(AggregateUnit<T1, AN1, AN2, AN3, AN4, AN5, AN6, AN7> lhs, AggregateUnit<T2, BN1, BN2, BN3, BN4, BN5, BN6, BN7> rhs){
		return AggregateUnit<Base, AN1+BN1, AN2+BN2, AN3+BN3, AN4+BN4, AN5+BN5, AN6+BN6, AN7+BN7>(T1::in_base_unit(lhs.val) * T2::in_base_unit(rhs.val));
	}	
	...
	template<typename T1, typename T2, int N1, int N2, int N3, int N4, int N5, int N6, int N7>
	bool operator==(AggregateUnit<T1, N1, N2, N3, N4, N5, N6, N7> lhs, AggregateUnit<T2, N1, N2, N3, N4, N5, N6, N7> rhs){
	...
	void stream_units(std::ostream& os,int pos, int N, Args... Ns){
		...
		if (N == 1)
		{
			os << unitnames[pos]; //change later to remove string dependency
		}
	...
	template<typename T, int N1, int N2, int N3, int N4, int N5, int N6, int N7>
	std::ostream& operator<<(std::ostream& os, AggregateUnit<T, N1, N2, N3, N4, N5, N6, N7> a){
